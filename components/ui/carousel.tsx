...
type CarouselContextProps = {
  carouselOptions?: EmblaOptionsType & { autoplay?: boolean };
...
>(({ className, dir, variant = "outline", size = "icon", ...props }, ref) => {
  ...
  return (
    <Button
      ...
      {...props}
    >
      <ChevronLeftIcon strokeWidth={3} className="h-6 w-6 text-[#23414D]" />
...
>(({ className, dir, variant = "outline", size = "icon", ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size={size}
      className={cn(
        "absolute z-10 hidden h-6 w-6 items-center justify-center rounded-full md:flex",
        orientation === "vertical"
      ...
      {...props}
    >
      <ChevronRightIcon className="h-6 w-6 text-[#23414D]" />
